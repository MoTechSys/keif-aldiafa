...
import { navLinks, getWhatsAppLink } from '@/data/constants'
    ...
    <>
          ...
          <nav className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <Link
                key={link.id}
                href={link.href}
                className={`transition-colors duration-200 hover:text-[#B8860B] text-sm ${pathname === link.href ? "text-[#B8860B]" : "text-[#F5F5DC]/80"}`}
              >
                {link.label}
              </Link>
            ))}
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
        >
          <motion.span animate={{ rotate: isMenuOpen ? 45 : 0 }} transition={{ duration: 0.3 }} className="text-lg">
            {isMenuOpen ? "✕" : "☰"}
          </motion.span>
          <span>القائمة</span>
        </motion.button>
